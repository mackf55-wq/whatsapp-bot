...
async function searchDropbox(address, month, day) {
  ...
  // Search Dropbox for files matching address
...
async function handlePhotoFlow(from, text, session) {
  ...
  if (!session.day) {
    // Waiting for day - now search Dropbox
    session.day = text;
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // Check if user is in a photo session
    if (sessions[from]) {
      await handlePhotoFlow(from, text, sessions[from]);
      return;
    }

    // Check if message starts with "photos"
